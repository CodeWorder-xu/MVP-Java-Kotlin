...
public class BookPageFactory {
    ...
    private PaintInfoUtil mPaintInfo;
    private ReadInfoUtil mReadInfo;
    private String percentStr;


    public BookPageFactory(Context context, int bookId) {
        BookFPresenterImpl presenter = new BookFPresenterImpl(context);
        BookBean bookBean = presenter.onAllBookList().get(bookId);
        mContext = context;
        mBookId = bookId;
        calWidthAndHeight();
        getFontFromAssets();
    ...
    private void initDatas(BookBean book) {
        ...
        PaintInfoUtil paintInfo = SaveUtils.getObject(mContext, SaveUtils.PAINT_INFO);
        if (paintInfo != null)
            mPaintInfo = paintInfo;
        else
            mPaintInfo = new PaintInfoUtil();
        ...
        ReadInfoUtil info = SaveUtils.getObject(mContext, mBookId + SaveUtils.DRAW_INFO);
        if (info != null)
            mReadInfo = info;
        else
            mReadInfo = new ReadInfoUtil();
    }
    ...
    private void getFontFromAssets() {
        ...
        try {
            fontNameList = assetManager.list(FontPopupView.FONTS);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for (int i = 0; i < fontNameList.length; i++) {
            ...
            String fontPath = FontPopupView.FONTS + "/" + fontNameList[i];
    ...
    public ReadInfoUtil getReadInfo() {
        return mReadInfo;
    }


    public PaintInfoUtil getPaintInfo() {
        return mPaintInfo;
    }

    public void setReadInfo(ReadInfoUtil readInfo) {
        mReadInfo = readInfo;
    }
